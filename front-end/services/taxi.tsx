...
const getComfortRIDEFare = (minutes: number, distance: number, pax: number) => {
  ...
  const priceTier: Prices = {
    name: "ComfortRide",
    fareType: "ComfortRIDE (Car / Taxi Flat Fare)",
    baseFare: 2.8,
    perKm: 0.5,
    perMin: 0.15,
  };
  let totalFare =
    priceTier.baseFare +
    priceTier.perKm * distance +
    priceTier.perMin * minutes;
  if (totalFare < 6.0) {
    totalFare = 6.0;
  }
  ...
  comfortRIDEFareMap.set(priceTier.name, [
    priceTier.fareType,
    minutes,
    totalFare,
  ]);
...
const getMeteredFare = (minutes: number, distance: number, pax: number) => {
  ...
  const priceTiers: Prices[] = [
    {
      name: "Metered Petrol Fuel Taxi",
      fareType: "Metered Hyundai i-40 Taxi",
      flagDown: 3.9,
      perDistance: 0.25,
      perTime: 0.25,
    },
    {
      name: "Metered Hybrid Taxi",
      fareType: "Metered Toyota Prius/Hyundai Ioniq Hybrid Taxis",
      flagDown: 4.1,
      perDistance: 0.25,
      perTime: 0.25,
    },
    {
      name: "Metered EV Taxi",
      fareType:
        "Metered Hyundai Ioniq/Hyundai Kona/BYD e6 Electric Vehicle Taxis",
      flagDown: 4.3,
      perDistance: 0.25,
      perTime: 0.25,
    },
    {
      name: "Metered Limousine Taxi (Limo)",
      fareType: "Metered Limousine Cab (4 Seater Limo)",
      flagDown: 4.1,
      perDistance: 0.35,
      perTime: 0.35,
    },
    {
      name: "Metered Limousine Taxi (MaxiCab)",
      fareType: "Metered Limousine Cab (7 Seater MaxiCab)",
      flagDown: 4.1,
      perDistance: 0.35,
      perTime: 0.35,
    },
  ...
  for (let i = 0; i < priceTiers.length; i++) {
    ...
    if (distance > 10) {
      distanceLeft -= 10;
      fare =
        priceTiers[i].flagDown +
        priceTiers[i].perDistance *
          (Math.ceil((10.0 - 1) / 0.4) + Math.ceil(distanceLeft / 0.35)) +
        priceTiers[i].perTime * Math.ceil(minutes * (60 / 45));
    } else {
      distanceLeft -= 1;
      fare =
        priceTiers[i].flagDown +
        priceTiers[i].perDistance * Math.ceil(distanceLeft / 0.4) +
        priceTiers[i].perTime * Math.ceil(minutes * (60 / 45));
    }
    ...
    // data: Promise<[serviceName, duration, fare]>[];
    meteredFareMap.set(priceTiers[i].name, [
      priceTiers[i].fareType,
      minutes,
      fare,
    ]);
...
const getLimoTransferFare = (
  ...
  const priceTiers: Prices[] = [
    {
      name: "LimoCab",
      fareType: "Limo Transfer Fare (4 Seater Limo)",
      hourlyRate: 50.0,
      lateNightSurcharge: 12.0,
      blockWaitingTime: 10.0,
    },
    {
      name: "MaxiCab",
      fareType: "Limo Transfer Fare (7 Seater MaxiCab)",
      hourlyRate: 55.0,
      lateNightSurcharge: 12.0,
      blockWaitingTime: 10.0,
    },
  ...
  for (let i = 0; i < priceTiers.length; i++) {
    ...
    if (currentHour >= 0 && currentHour < 6) {
      fare =
        priceTiers[i].hourlyRate * tripHours +
        priceTiers[i].lateNightSurcharge * tripHours;
    } else {
    ...
    // data: Promise<[serviceName, duration, fare]>[];
    limousineCabFareMap.set(priceTiers[i].name, [
      priceTiers[i].fareType,
      minutes,
      fare,
    ]);
...
export default class Taxi {
  ...
  async getData(
    ...
    pax: number = 1
  ): Promise<Promise<[string, number, number]>[]> {
    ...
    await googlemap
      ...
      .then((drivingData: [number, number]) => {
    ...
    combinedFareMap.forEach((value, key) => {
      arrayOfPromise.push(
        Promise.resolve([value[0], value[1], value[2].toFixed(2)])
      );
    });
